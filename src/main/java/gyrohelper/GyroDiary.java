...
public class GyroDiary {
  ...
  private Object m_gyroDataSynchronizer = new Object();
  ...
  private void add(GyroReading gyroReading) {
    ...
    synchronized (m_gyroDataSynchronizer) {
  ...
  public double getHeadingAtTimeStamp(long timeStamp) {
    ...
    synchronized (m_gyroDataSynchronizer) {
      try {
        long earliestTimeStamp = m_gyroData.getFirst().timeStamp;
        if (timeStamp > earliestTimeStamp) {

          heading = m_gyroData.getFirst().angle;
          System.out.println(String.format(
              "Couldn't determine heading from timeStamp %d, it's too young. Returned youngest heading of %f from timestamp %d.",
              timeStamp, heading, earliestTimeStamp));
          /*
      ...
      try {
        long oldestTimeStamp = m_gyroData.getLast().timeStamp;
        if (timeStamp < oldestTimeStamp) {
          heading = m_gyroData.getLast().angle;
          System.out.println(String.format(
              "Couldn't determine heading from timeStamp %d, it's too old. Returned oldest heading of %f from timestamp %d.",
              timeStamp, heading, oldestTimeStamp));
          return heading;
        }
      } catch (Exception e) {
        /*
